<template>
  <div class="kpiDis">
    <Title @goBack="goBack" title="绩效评估分布">
      <div class="btn">
        <button
    ...
    <div v-if="currentTab === '经管'">
      <TableTitle class="tableTitle" name="全部数据" img="true"></TableTitle>
      <div class="staff-row">
        <div class="row-staff" @click="showPopFlag('Gallery',emStaff)">
          <Gallery :dataGallery="emStaff" :id="emStaff.id"></Gallery>
        </div>
      </div>
...
export default {
  ...
  created() {
      ...
      .then(response => {
        ...
        Object.keys(response.companymap)
          ...
          .forEach(element => {
            ...
            Object.keys(response.companymap[element])
              .reverse()
              .forEach((key, i) => {
                Object.keys(response.companymap[element][key]).forEach(
                  value => {
    ...
    //经管
    this.$http
      .get("/companyperformancedata/selectPerformanceAssessAdmin")
      .then(response => {
        Object.keys(response.allmap)
          .reverse()
          .forEach((element, i) => {
            Object.keys(response.allmap[element]).forEach(key => {
              this.emStaff.Y[i].push(response.allmap[element][key]);
            });
          });
